...
start "" /b powershell -NoProfile -WindowStyle Hidden -Command "$ready=$false; 1..30 | ForEach-Object { if (-not $ready) { Start-Sleep 1; try { $v=Invoke-RestMethod 'http://127.0.0.1:%PORT%/api/version' -TimeoutSec 1; if ($v.build -eq '20260806.1') { $ready=$true; Start-Process 'http://127.0.0.1:%PORT%/?build=20260806.1' } } catch {} } }"
